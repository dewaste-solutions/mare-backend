...
export async function createInvitationToken(
	...
	try {
		...
		await db.transaction(async (tx) => {
			...
			const invitationResult = await tx.insert(oneTimeTokens).values({
			...
			const statusResult = await tx.select({id: statuses.id}).from(statuses).where(
				eq(statuses.name, "invited"))
