...
import { db } from "../../db";
import { permissions, refreshTokens, rolePermissions, roles, sessions, users } from "../../db/schema/auth";
...
export const getAccessToken = async (req: Request, res: Response) => {
	try {
		...
		const record = await db
			...
			.from(refreshTokens)
			.innerJoin(sessions, eq(refreshTokens.sessionId, sessions.id))
			.where(and(eq(refreshTokens.token, refreshTokenCookies), gt(sessions.notAfter, now)))
        ...
        if (decodedRefreshToken === null || typeof decodedRefreshToken !== 'object' || decodedRefreshToken.email === undefined) {
            res.status(401).json({ message: "Unauthorized" });
            return
        }
